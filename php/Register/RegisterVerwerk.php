...
<main>
    ...
    if (isset($login) && ($login == true)) {
        ...
        if (isset($_POST['PasswordConfirm']) && !empty($_POST['PasswordConfirm'])) {
            $PasswordConfirmFilled = true;
        } else {
            echo "<p class='error'>Password conformeer is niet ingevuld.</p>";
        }
        if (isset($_POST['Email']) && !empty($_POST['Email'])) {
            $EmailFilled = true;
        } else {
            echo "<p class='error'>Email is niet ingevuld.</p>";
        }
        if (isset($_POST['EmailConfirm']) && !empty($_POST['EmailConfirm'])) {
            $EmailConfirmFilled = true;
        } else {
            echo "<p class='error'>Email conformeer is niet ingevuld.</p>";
        }
    ...
    if (isset($SubmitFilled) && $SubmitFilled == true &&
        ...
        isset($PasswordConfirmFilled) && $PasswordConfirmFilled == true &&
        isset($EmailFilled) && $EmailFilled == true &&
        isset($EmailConfirmFilled) && $EmailConfirmFilled == true ) {
    ...
    if (isset($TokenCheck) && $TokenCheck == true ) {
        ...
        /*
         * check of Email klein of groot genoeg is
         */
        if (strlen($_POST['Email']) <= 50 && strlen($_POST['Email']) >= 6) {
            $PasswordLength = true;
        } else {
            echo "<p class='error'>Password is niet groter dan 8 characters of kleiner dan 40 characters.</p>";
        }
